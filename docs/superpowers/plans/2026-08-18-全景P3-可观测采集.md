...
- e2e 应用 `hist-e2e-fixed`（2 条 success 历史行）保留在 .28 作展示样本。**删应用的级联语义有分界**：
  - 000038（表完整性约束）上线**前**删除应用**不会**清 deploy_history——历史行残留为孤儿，
    仍会出现在统计聚合里（此前已删的应用即留有残留统计行，属已知历史数据，不回溯清理）；
  - 000038 上线**后**删除应用由 FK ON DELETE CASCADE 级联清 deploy_history（与其余 5 张
    app 关联表对齐），再删 `hist-e2e-fixed` 即连历史一并清掉。
