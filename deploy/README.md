...
| scp 增量残留孤儿         | `next build` 报 `Failed to type check`，旧容器不换（仍 Up hours 跑旧前端） | 方式 A 只推改动文件，不删 .28 上 repo 已删的残留 `.tsx`（如 `context-drawer.tsx`）；`next build` 类型检查覆盖 tsconfig 下**所有** `.tsx`（含不可达孤儿），孤儿 import 已删模块即挂。**删文件类改动必须同步 `ssh rm` 残留**，且判构建成败看容器 uptime 回落秒级 + `Successfully built`，**别 `docker-compose ... \| tail`**（tail 恒退出 0 掩盖真实失败） |
