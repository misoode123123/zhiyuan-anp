...
export default function AttendancePage() {
  ...
  const load = (id: string) => {
    if (!id) return;
    fetch(`${API_BASE_URL}/project-spaces/${id}/attendance/mine`)
      .then((r) => r.json())
      .then((r: Envelope<Rec[]>) => setMine(r.data ?? []));
    fetch(`${API_BASE_URL}/attendance/inbox`)
      .then((r) => r.json())
      .then((r: Envelope<Rec[]>) => setInbox(r.data ?? []));
  };
  useEffect(() => {
    load(psID);
  }, [psID]);
  ...
  async function decide(id: string, ok: boolean) {
    const res = await fetch(`${API_BASE_URL}/attendance/${id}/${ok ? "approve" : "reject"}`, {
      method: "POST",
    });
    const r = await res.json();
    ...
    <div>
      <div className="mb-4 flex items-center gap-3">
        ...
        <span className="ml-auto text-xs text-neutral-500">
          当前用户：{me}（侧栏可切换，模拟员工/上级）
        </span>
      </div>
      <p className="mb-4 text-sm text-neutral-600">
        员工提交考勤（休息/加班/请假）→ 转直接上级审批收件箱 →
        批准/驳回。本模块由平台自身流水线（opencode）产出。
      </p>
            ...
            {mine.map((r) => (
              <div key={r.id} className="rounded-md border border-neutral-200 bg-white p-2 text-sm">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded bg-blue-100 px-1.5 py-0.5 text-xs">
                    {STATUS_LABEL[r.status] ?? r.status}
                  </span>
                  <span
                    className={`rounded px-1.5 py-0.5 text-xs ${APPROVAL_COLOR[r.approval_status] ?? "bg-neutral-100"}`}
                  >
                    {r.approval_status}
                  </span>
              ...
              <div key={r.id} className="rounded-md border border-neutral-200 bg-white p-2 text-sm">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    ...
                    className="ml-auto rounded bg-emerald-100 px-2 py-0.5 text-xs text-emerald-700"
                  >
                    批准
                  </button>
                  <button
