...
export default function TestingPage() {
  ...
  async function generate(rid: string) {
    ...
    setMsg("");
    try {
      const res = await fetch(`${API_BASE_URL}/project-spaces/${psID}/requirements/${rid}/generate-tests`, { method: "POST" });
      const r = await res.json();
      setMsg(r.code === 0 ? `✓ 已生成 ${r.data?.length ?? 0} 条测试用例（含可执行 HTTP 检查）` : `✗ ${r.message}`);
      load(psID);
    } catch (e) {
      setMsg(String(e));
    } finally {
  ...
  async function runOne(tcid: string) {
    ...
    setMsg("");
    try {
      const res = await fetch(`${API_BASE_URL}/project-spaces/${psID}/test-cases/${tcid}/run`, { method: "POST" });
      const r = await res.json();
      if (r.code === 0) {
        const tc: TC = r.data;
        setMsg(`「${tc.title}」→ ${STATUS_LABEL[tc.status] ?? tc.status}（实际 ${tc.actual_status || "—"}）`);
      } else {
        setMsg(`✗ ${r.message}`);
      }
      load(psID);
  ...
  async function runAll(rid: string) {
    ...
    setMsg("");
    try {
      const res = await fetch(`${API_BASE_URL}/project-spaces/${psID}/requirements/${rid}/run-tests`, { method: "POST" });
      const r = await res.json();
      if (r.code === 0) {
        const d = r.data;
        setMsg(`批量验收完成：✓ ${d.passed} 通过 · ✗ ${d.failed} 失败 · ⊙ ${d.manual} 人工（共 ${d.total}）${d.base_url ? ` · 被测 ${d.base_url}` : " · 未归属已部署应用，HTTP 用例标为人工"}`);
      } else {
        setMsg(`✗ ${r.message}`);
      }
      load(psID);
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-sm text-neutral-600">AI 把需求验收标准转为可执行测试用例，并对着<b>已部署应用的 URL</b> 自动发请求验收（状态码 + 响应体比对）。</p>
          ...
          {reqs.map((r) => (
            <div key={r.id} className="flex items-center justify-between rounded-md border border-neutral-200 bg-white p-2 text-sm">
              <div className="flex items-center gap-2">
                <span>{r.title}</span>
                {r.application_id
                  ? <span className="rounded bg-purple-100 px-1.5 py-0.5 text-xs text-purple-700">📦 已归属应用</span>
                  : <span className="rounded bg-neutral-100 px-1.5 py-0.5 text-xs text-neutral-500">未归属应用</span>}
                {casesByReq.get(r.id)?.length
                  ? <span className="text-xs text-neutral-400">· {casesByReq.get(r.id)!.length} 用例</span>
                  : null}
              </div>
              <div className="flex gap-1">
                <button onClick={() => generate(r.id)} disabled={!!genID} className="rounded bg-blue-600 px-2 py-1 text-xs text-white disabled:opacity-50">
                  {genID === r.id ? "生成中…" : "生成测试用例"}
                </button>
                {casesByReq.get(r.id)?.length ? (
                  <button onClick={() => runAll(r.id)} disabled={!!runReqID} className="rounded bg-emerald-600 px-2 py-1 text-xs text-white disabled:opacity-50">
              ...
              <div key={c.id} className="rounded-md border border-neutral-200 bg-white p-3 text-sm">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`rounded px-1.5 py-0.5 text-xs ${STATUS_STYLE[c.status] ?? "bg-neutral-100"}`}>{STATUS_LABEL[c.status] ?? c.status}</span>
                  </div>
                  <button onClick={() => runOne(c.id)} disabled={!!runID} className="rounded bg-neutral-800 px-2 py-1 text-xs text-white disabled:opacity-50">
                    {runID === c.id ? "运行中…" : "▶ 运行"}
                  </button>
                </div>
                ...
                {hasHTTP && (
                  <div className="mt-1.5 rounded bg-neutral-50 px-2 py-1 font-mono text-xs text-neutral-700">
                    ...
                    {c.expected_status ? <> · 期望 <b>{c.expected_status}</b></> : null}
                    {c.expected_body ? <> · 响应含 <b>"{c.expected_body}"</b></> : null}
                  </div>
                )}
                ...
                {c.run_at && (
                  <div className="mt-1.5 rounded bg-neutral-50 px-2 py-1 text-xs">
                    ...
                    <b className={c.status === "passed" ? "text-emerald-700" : c.status === "failed" ? "text-red-700" : "text-amber-700"}>
                      实际 {c.actual_status || "—"}
                    </b>
                    {c.actual_body && <span className="ml-1 text-neutral-500">· {c.actual_body.slice(0, 160)}</span>}
                  </div>
                )}
                <div className="mt-1 text-[11px] text-neutral-300">归属：{reqMap.get(c.requirement_id)?.title ?? c.requirement_id}</div>
              </div>
            );
          })}
          {cases.length === 0 && <div className="text-sm text-neutral-400">暂无测试用例（先对某需求「生成测试用例」）</div>}
        </div>
      </div>
    </div>
  );
