  ...
  return (
    <aside className="w-64 shrink-0 overflow-y-auto border-r border-neutral-200 bg-neutral-50 p-2 text-xs">
      <div className="mb-2 flex items-center justify-between">
        ...
        <button onClick={onClose} className="text-neutral-400 hover:text-neutral-700" title="折叠">
          ◀
        </button>
      </div>
              ...
              detail.requirements!.map((q) => (
                <div key={q.id}>
                  ...
                  >
                    ...
                    {q.priority && (
                      <span
                        className={`shrink-0 rounded px-1 text-[10px] ${q.priority === "P0" ? "bg-red-100 text-red-700" : q.priority === "P2" ? "bg-neutral-200 text-neutral-600" : "bg-blue-100 text-blue-700"}`}
                        title="需求等级"
                      >
                        {q.priority}
                      </span>
                    ...
                    <span
                      className={`truncate ${selectedReq === q.id ? "font-semibold text-blue-700" : "text-neutral-700"}`}
                    >
                      {q.title || "(无标题)"}
                    </span>
                  </button>
                  {q.assignee && selectedReq !== q.id ? (
                    <span className="shrink-0 text-[10px] text-neutral-400" title="已被认领">
                      {q.assignee} 开发中
                    </span>
                  ) : selectedReq !== q.id ? (
                    <button
                  ...
                  {openReq === q.id && (
                    <div className="mb-1 ml-3 space-y-0.5 border-l border-neutral-300 pl-2 text-[11px] text-neutral-600">
                      {q.fixed_version && (
                        <div className="text-neutral-500">📦 计划版本:{q.fixed_version}</div>
                      )}
                      ...
                      {q.acceptance_criteria &&
                        (() => {
                          let arr: string[] = [];
                          try {
                            arr = JSON.parse(q.acceptance_criteria as string);
                          } catch {
                            arr = [q.acceptance_criteria as string];
                          }
                          return (
                            <div>
                              {arr.map((c, i) => (
              ...
              detail.changes!.map((c) => (
                <div key={c.id}>
                  <div className="flex py-0.5">
                    <button
                      onClick={() => setOpenChg(openChg === c.id ? null : c.id)}
                      className="flex gap-1 text-left"
                    >
                      <span>{STATUS_DOT[c.status] ?? "•"}</span>
                      <span>
                        {c.kind} · <span className="text-neutral-500">{c.status}</span>
                      </span>
                    </button>
                    {c.status === "pending" && (
                  ...
                  {openChg === c.id && (
                    <pre className="mb-1 ml-3 max-h-32 overflow-auto whitespace-pre-wrap border-l border-neutral-300 pl-2 text-[11px] text-neutral-600">
                      {c.output || "(无内容)"}
                    </pre>
                  )}
                </div>
            ...
            {detail.releases?.length ? (
              detail.releases!.map((r) => (
                <div key={r.id} className="py-0.5">
                  <span>{STATUS_DOT[r.status] ?? "•"}</span> v{r.version} ·{" "}
