...
export default function DocsPage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/docs`).then((r) => r.json()).then((r: Envelope<Doc[]>) => setAll(r.data ?? []));
  }, []);

  const filtered = q.trim()
    ? all.filter((d) => (d.title + " " + d.summary + " " + d.path).toLowerCase().includes(q.toLowerCase()))
    : all;
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-sm text-neutral-600">系统所有方案与设计文档（docs/），可搜索、可阅读。</p>
        ...
        <div className="rounded-lg border border-neutral-200 bg-white p-3">
          ...
          <div className="max-h-[72vh] space-y-3 overflow-auto">
            {Object.entries(grouped).map(([cat, list]) => (
              <div key={cat}>
                <div className="mb-1 text-xs font-semibold text-neutral-500">{cat}（{list.length}）</div>
                {list.map((d) => (
                  <button
                    key={d.path}
                    onClick={() => open(d)}
