...
export default function GovernancePage() {
  ...
  const loadStds = () => {
    fetch(`${API_BASE_URL}/standards`).then((r) => r.json()).then((r: Envelope<Std[]>) => setGlobalStds(r.data ?? []));
    if (psID) fetch(`${API_BASE_URL}/project-spaces/${psID}/standards`).then((r) => r.json()).then((r: Envelope<Std[]>) => setProjStds(r.data ?? []));
  };
  useEffect(() => {
    loadStds();
  }, [psID]);
  ...
  async function submitStd() {
    ...
    const url = stdScope === "global" ? `${API_BASE_URL}/standards` : `${API_BASE_URL}/project-spaces/${psID}/standards`;
  ...
  const stdList = (title: string, list: Std[]) => (
    <div className="mb-5">
      <div className="mb-2 text-sm font-semibold text-neutral-700">{title}（{list.length}）</div>
      <div className="space-y-2">
        {list.map((s) => (
          <div key={s.id} className={`rounded-md border border-neutral-200 bg-white p-3 text-sm ${s.enabled ? "" : "opacity-50"}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className={`rounded px-1.5 py-0.5 text-xs ${STD_CAT_COLOR[s.category] ?? STD_CAT_COLOR.general}`}>{s.category}</span>
                <span className="font-medium">{s.name}</span>
                <span className="text-xs text-neutral-400">prio {s.priority}</span>
              </div>
              <div className="flex gap-2">
                <button onClick={() => toggleStd(s)} className="text-xs text-blue-600">{s.enabled ? "禁用" : "启用"}</button>
                <button onClick={() => deleteStd(s.id)} className="text-xs text-red-600">删除</button>
              </div>
            </div>
      ...
      {tab === "rules" && (
        <>
          <div className="mb-1 flex items-center justify-between">
            ...
            <button onClick={() => setShowForm(!showForm)} className="rounded-md bg-blue-600 px-3 py-1.5 text-sm text-white">
              {showForm ? "取消" : "＋ 新建规则"}
            </button>
          </div>
      ...
      {tab === "standards" && (
        <div>
          <p className="mb-3 text-sm text-neutral-600">
            编码规范 = <b>注入式生成指导</b>：编码时拼进 prompt 告诉 AI「怎么写」（全局 + 项目级叠加）；与 RaC 规则（硬约束/block）互补。
          </p>
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            <button onClick={previewEffective} className="rounded-md bg-neutral-700 px-3 py-1 text-xs text-white">预览生效规范</button>
          </div>

          {effPreview !== null && (
            <pre className="mb-4 max-h-48 overflow-auto whitespace-pre-wrap rounded bg-neutral-900 p-3 text-xs text-green-300">{effPreview || "(无生效规范)"}</pre>
          )}
