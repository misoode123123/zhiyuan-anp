...
const cases: { name: string; input: DevWizardInput; current: string; hint: string }[] = [
  {
    name: "全新无镜像",
    input: I("", []),
    current: "code",
  ...
  {
    name: "test running 无 prod",
    input: I("img:v1", [{ env: "test", status: "running" }]),
    current: "prod",
    hint: "test 已跑通，点「🚀上线」到 prod",
  },
  {
    name: "prod running 全绿",
    input: I("img:v1", [{ env: "prod", status: "running" }]),
    current: "done",
    hint: "已上线 ✅，可继续编码迭代",
  },
  {
    name: "test 非 running 不算完成",
    input: I("img:v1", [{ env: "test", status: "building" }]),
    current: "test",
    hint: "代码已构建，点「构建部署(test)」验证",
  },
  {
    name: "test stopped 不算完成",
    input: I("img:v1", [{ env: "test", status: "stopped" }]),
    current: "test",
    hint: "代码已构建，点「构建部署(test)」验证",
  },
];
