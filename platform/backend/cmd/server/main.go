...
func main() {
	...
	mwReconciler := mwsupply.NewReconciler(mwStore, appDeployStore, mwProbe, mwProbe, mwsupply.NewOSDocker(), cfg.AppDeployHost, pgProvisioner, instanceMgr, quotaSvc) // appDeployStore 满足 mwsupply.EnvWriter；pgProvisioner 给 pg shared 自管供给；instanceMgr 给 pg dedicated（满足 PgDedicatedRunner）；quotaSvc 给 P4 dedicated 实例数配额（满足 DedicatedQuotaChecker）
