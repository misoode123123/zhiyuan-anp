...
const reqCols = `id, project_space_id, COALESCE(application_id,'') AS application_id, title, description, user_story, acceptance_criteria, status, created_at, updated_at`

func (r *Repository) Create(ctx context.Context, req *Requirement) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO requirement (id, project_space_id, application_id, title, description, user_story, acceptance_criteria, status)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		req.ID, req.ProjectSpaceID, req.ApplicationID, req.Title, req.Description, req.UserStory, req.AcceptanceCriteria, req.Status)
	return err
}
